...
def init_distributed(args):
    ...
    if args.train_engine == 'torch_ddp':
        ...
        if deepspeed is None:
            raise RuntimeError("deepspeed engine requires deepspeed; install with: pip install deepspeed")
...
def init_optimizer_and_scheduler(args, configs, model, gan):
    if gan is False:
        backbone_lr = configs['train_conf'].get('backbone_lr', None)
        base_model = model.module if hasattr(model, 'module') else model
        emotion_prefixes = getattr(base_model, '_emotion_prefixes', ())

        if backbone_lr is not None and emotion_prefixes:
            emo_params, backbone_params = [], []
            for name, p in base_model.named_parameters():
                if not p.requires_grad:
                    continue
                if any(name.startswith(pf) for pf in emotion_prefixes):
                    emo_params.append(p)
                else:
                    backbone_params.append(p)
            optim_conf = dict(configs['train_conf']['optim_conf'])
            emo_lr = optim_conf.pop('lr')
            param_groups = [
                {'params': emo_params, 'lr': emo_lr},
                {'params': backbone_params, 'lr': backbone_lr},
            ]
            logging.info(f'[diff_lr] emotion={len(emo_params)} params lr={emo_lr}, '
                         f'backbone={len(backbone_params)} params lr={backbone_lr}')
...
def cosyvoice_join(group_join, info_dict):
    ...
    if info_dict["batch_idx"] != 0:
        ...
        try:
            _timeout = getattr(getattr(group_join, 'options', None), '_timeout', datetime.timedelta(seconds=300))
